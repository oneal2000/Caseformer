...
class DenseModel(nn.Module):
    ...
    def inference(
        ...
        effective_bsz = self.train_args.per_device_eval_batch_size
        ...
        # print(effective_bsz)
        # print(q_reps,len(q_reps))
        # print(p_reps,len(p_reps))
    ...
    def mean_inf(
        ...
        query_id = query['input_ids'][0]
        query_mask = query['attention_mask'][0]

        ids = []
        masks = []

        start_q = 0
        while start_q < len(query_id):
            if start_q + 512 > len(query_id):
                break
            else:
                end_q = start_q + 512      

            ids.append(query_id[start_q:end_q])   
            masks.append(query_mask[start_q:end_q])

            start_q += step

        f_qids = torch.stack(ids,dim=0)
        f_masks = torch.stack(masks,dim=0)

        f_query = {}
        f_query['input_ids'] = f_qids
        f_query['attention_mask'] = f_masks
    

        '''
        
        '''
        psg_id = passage['input_ids'][0]
        psg_mask = passage['attention_mask'][0]

        ids = []
        masks = []

        start_p = 0
        while start_p < len(psg_id):
            if start_p + 512 > len(psg_id):
                break
            else:
                end_p = start_p + 512      

            ids.append(psg_id[start_p:end_p])   
            masks.append(psg_mask[start_p:end_p])

            start_p += step

        f_pids = torch.stack(ids,dim=0)
        f_pmasks = torch.stack(masks,dim=0)

        f_psg = {}
        f_psg['input_ids'] = f_pids
        f_psg['attention_mask'] = f_pmasks



        q_hidden, q_reps = self.encode_query(f_query)
        p_hidden, p_reps = self.encode_passage(f_psg)

        mean_q = torch.mean(q_reps,dim=0)
        mean_p = torch.mean(p_reps,dim=0)
        ...
        # m1 = torch.matmul(mean_q, mean_q)
        # m2 = torch.matmul(mean_p, mean_p)

        # m1 = math.sqrt(m1)
        # m2 = math.sqrt(m1)


        # score = torch.matmul(mean_q, mean_p)
        score = F.cosine_similarity(mean_q, mean_p, dim=0)
