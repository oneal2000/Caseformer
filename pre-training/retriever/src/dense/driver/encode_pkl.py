...
def main():
    ...
    parameters = torch.load(model_args.model_name_or_path, map_location=lambda storage, loc: storage)

    model = DenseModelForInference.load_state_dict(parameters['model'])
    ...
    # model.load_static_dict(torch.load(model_args.model_name_or_path))
    model.eval()
